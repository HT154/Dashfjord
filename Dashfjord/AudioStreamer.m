...
//
static void ASPropertyListenerProc(void *inClientData, AudioFileStreamID inAudioFileStream, AudioFileStreamPropertyID inPropertyID, UInt32 *ioFlags) {
...
//
static void ASPacketsProc(void *inClientData, UInt32 inNumberBytes, UInt32 inNumberPackets, const void *inInputData, AudioStreamPacketDescription *inPacketDescriptions) {
...
- (BOOL)isFinishing {
	@synchronized (self) {
		if ((errorCode != AS_NO_ERROR && state != AS_INITIALIZED) || ((state == AS_STOPPING || state == AS_STOPPED) && stopReason != AS_STOPPING_TEMPORARILY)) {
			return YES;
		}
	}
...
- (void)failWithErrorCode:(AudioStreamerErrorCode)anErrorCode {
	@synchronized(self) {
		...
		if (err) {
			...
			NSLog(@"%@ err: %c%c%c%c %d\n", [AudioStreamer stringForErrorCode:anErrorCode], errChars[3], errChars[2], errChars[1], errChars[0], (int)err);
		} else {
...
- (BOOL)isWaiting {
	@synchronized(self) {
		if ([self isFinishing] || state == AS_STARTING_FILE_THREAD|| state == AS_WAITING_FOR_DATA || state == AS_WAITING_FOR_QUEUE_TO_START || state == AS_BUFFERING){
			return YES;
		}
	}
...
- (BOOL)openReadStream {
	@synchronized(self) {
		...
		if (fileLength > 0 && seekByteOffset > 0) {
			CFHTTPMessageSetHeaderFieldValue(message, CFSTR("Range"), (__bridge CFStringRef)[NSString stringWithFormat:@"bytes=%ld-%ld", (long)seekByteOffset, (long)fileLength]);
			discontinuous = YES;
		}
...
- (void)stop {
	@synchronized(self) {
		if (audioQueue && (state == AS_PLAYING || state == AS_PAUSED || state == AS_BUFFERING || state == AS_WAITING_FOR_QUEUE_TO_START)) {
			self.state = AS_STOPPING;
...
#pragma clang diagnostic pop
		...
		if (!audioFileStream) {
			...
			AudioFileTypeID fileTypeHint = [AudioStreamer hintForFileExtension:self.fileExtension];

			// create an audio file stream parser
			err = AudioFileStreamOpen((__bridge void * _Nullable)(self), ASPropertyListenerProc, ASPacketsProc, fileTypeHint, &audioFileStream);
			if (err) {
				[self failWithErrorCode:AS_FILE_STREAM_OPEN_FAILED];
				return;
			}
...
- (void)handlePropertyChangeForQueue:(AudioQueueRef)inAQ propertyID:(AudioQueuePropertyID)inID {
	if (![[NSThread currentThread] isEqual:internalThread]) {
		[self performSelector:@selector(handlePropertyChange:) onThread:internalThread withObject:@(inID) waitUntilDone:NO modes:@[NSDefaultRunLoopMode]];
		return;
	}
