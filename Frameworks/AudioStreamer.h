...
typedef NS_ENUM(unsigned int, AudioStreamerState) {
	AS_INITIALIZED = 0,
...
typedef NS_ENUM(unsigned int, AudioStreamerStopReason) {
	AS_NO_STOP = 0,
	AS_STOPPING_EOF,
...
typedef NS_ENUM(unsigned int, AudioStreamerErrorCode) {
	AS_NO_ERROR = 0,
...
@property (getter=isAborted, readonly) BOOL aborted; // return YES if streaming halted due to error (AS_STOPPING + AS_STOPPING_ERROR)
- (void)seekToTime:(double)newSeekTime;
@property (readonly) double calculatedBitRate;
