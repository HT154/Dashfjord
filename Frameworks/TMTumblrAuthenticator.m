...
typedef void (^NSURLSessionCompletionHandler)(NSData *, NSURLResponse *, NSError *);
...
static NSURLSession *authenticationSession;

+ (id)sharedInstance {
    static TMTumblrAuthenticator *instance;
    static dispatch_once_t predicate;
    dispatch_once(&predicate, ^{
        instance = [[TMTumblrAuthenticator alloc] init];
        authenticationSession = [NSURLSession sessionWithConfiguration:[NSURLSessionConfiguration defaultSessionConfiguration]];
    });
    return instance;
}
...
- (void)authenticate:(NSString *)URLScheme callback:(TMAuthenticationCallback)callback {
    ...
    NSURLSessionCompletionHandler handler = ^(NSData *data, NSURLResponse *response, NSError *error) {
        if (error) {
            if (callback) {
    ...
    [[authenticationSession dataTaskWithRequest:request completionHandler:handler] resume];
}
...
- (void)xAuth:(NSString *)emailAddress password:(NSString *)password callback:(TMAuthenticationCallback)callback {
    ...
    [authenticationSession dataTaskWithRequest:request completionHandler:handler];
}
